...
- днем будет идти точечный апдейт каждые 3 часа без очистки текущей `BD`;
- ночной полный проход стартует около 21:00;
- проверка количества авто в `BD` против счетчика сайта запускается около 23:50.

Технически Apps Script запускает триггеры с небольшим допуском по времени, поэтому `21:00` и `23:50` означают запуск примерно около этого времени.

Если вы уже запускали `setupCrystalMotorsSync` и в `sync_log` есть только строка `setup / OK`, просто выберите функцию `startFullCatalogRefresh` и нажмите Run. Именно она запускает полный сбор `BD`.
...
- во время ночного полного прохода не обнуляет `BD`, а заменяет ее полной базой только после завершения сбора;
- прогресс виден в `sync_log` как `refresh_batch / CONTINUE`.

Дневной апдейт (`incrementalRefreshCrystalMotorsCatalog`) смотрит первые страницы каталога, проверяет дубли по `url`, добавляет новые авто и обновляет найденные, но не удаляет старые строки. Удаления и полная сверка происходят только ночным полным проходом.

Основной проход сразу сохраняет первое фото из карточки списка. Полные характеристики (`Пробег`, `Кузов`, `Двигатель`, `Привод`, `Мощность`, `КПП`, `Руль`) дозаполняются отдельными пакетами через меню `CM66 авто -> Дозаполнить фото и характеристики`, потому что для них нужно открывать страницы конкретных авто. Если нужно принудительно пройти все карточки заново, используйте `CM66 авто -> Заново пройти фото и характеристики`.
