...
- обновление будет идти примерно раз в 1,5 часа.

Технически Apps Script не умеет штатный триггер ровно на 90 минут, поэтому скрипт ставит 30-минутный триггер `scheduledRefreshCrystalMotorsCatalog`, а внутри пропускает запуск, если с прошлого обновления прошло меньше 90 минут.
...
- за один запуск обрабатывает `PAGES_PER_RUN` страниц;
- сохраняет промежуточный результат в скрытую вкладку `sync_buffer`;
- следующий запуск продолжает с сохраненной страницы;
- в `BD` пишет текущий частичный результат после каждой пачки, а после завершения заменяет его полной базой;
- прогресс виден в `sync_log` как `refresh_batch / CONTINUE`.

Основной проход сразу сохраняет первое фото из карточки списка. Полные характеристики (`Пробег`, `Кузов`, `Двигатель`, `Привод`, `Мощность`, `КПП`, `Руль`) дозаполняются отдельными пакетами через меню `CM66 авто -> Дозаполнить фото и характеристики`, потому что для них нужно открывать страницы конкретных авто.

Если нужно начать полный проход заново, используйте меню `CM66 авто -> Сбросить прогресс обновления`.
